...
export const getSongData = async (
  ...
  // Validate instrument explicitly since it's interpolated into the prompt
  // In a client-side only app, type assertions can be bypassed at runtime
  const validInstruments: Instrument[] = ['guitar', 'ukulele', 'piano'];
  const safeInstrument = validInstruments.includes(instrument) ? instrument : 'guitar';
