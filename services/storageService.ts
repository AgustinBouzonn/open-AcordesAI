...
const getSongsCache = (): Record<string, Song> => {
  if (!memoryCache.songs) {
    ...
    memoryCache.songs = stored ? safeJSONParse<Record<string, Song>>(stored, {}) : {};
  }
...
const getFavoritesCache = (): string[] => {
  if (!memoryCache.favorites) {
    ...
    memoryCache.favorites = stored ? safeJSONParse<string[]>(stored, []) : [];
  }
...
const getHistoryCache = (): string[] => {
  if (!memoryCache.history) {
    ...
    memoryCache.history = stored ? safeJSONParse<string[]>(stored, []) : [];
  }
...
const getCommentsCache = (): Record<string, Comment[]> => {
  if (!memoryCache.comments) {
    ...
    memoryCache.comments = stored ? safeJSONParse<Record<string, Comment[]>>(stored, {}) : {};
  }
