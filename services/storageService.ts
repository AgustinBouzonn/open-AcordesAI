...
export const getFavoriteSongsFull = (): Song[] => {
  ...
  const cache = getSongsCache();
  // ⚡ Bolt Performance Optimization: Replace .map().filter(Boolean) with a single .reduce() pass
  // Reduces intermediate array allocations and iteration overhead (O(N) vs O(2N))
