...
import { safeJSONParse, safeSetItem } from '../utils/security';
...
const getSongsCache = (): Record<string, Song> => {
  if (!memoryCache.songs) {
    ...
    memoryCache.songs = (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) ? parsed : {};
  }
...
const getFavoritesCache = (): string[] => {
  if (!memoryCache.favorites) {
    ...
    const parsed = safeJSONParse(stored, []);
    memoryCache.favorites = Array.isArray(parsed) ? parsed : [];
  }
...
const getHistoryCache = (): string[] => {
  if (!memoryCache.history) {
    ...
    const parsed = safeJSONParse(stored, []);
    memoryCache.history = Array.isArray(parsed) ? parsed : [];
  }
...
const getCommentsCache = (): Record<string, Comment[]> => {
  if (!memoryCache.comments) {
    ...
    const parsed = safeJSONParse(stored, {});
    memoryCache.comments = (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) ? parsed : {};
  }
