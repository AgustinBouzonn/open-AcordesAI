...
async def run():
    async with async_playwright() as p:
        ...
        await page.evaluate(f"""
            localStorage.setItem('acordesai_songs_cache', `{mock_song_json}`);
            // Also need to trigger storage event or reload for the service to pick it up?
            // The service checks cache on loadSong, which calls getCachedSong.
            // But we need to make sure the in-memory cache is invalidated if we just wrote to LS.
            // Or easier, just reload the page after setting LS.
        """)

        print("Reloading page to apply localStorage...")
        await page.reload()
