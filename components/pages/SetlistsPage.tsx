          ...
          {list.map((sl) => (
            <div key={sl.id} className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
              <div className="p-4 flex items-center gap-3">
                ...
                {renamingId === sl.id ? (
                  <>
                    <button onClick={() => void submitRename(sl.id)} className="p-2 text-emerald-400 hover:text-emerald-300" aria-label="Guardar nombre" title="Guardar nombre"><Check size={16} /></button>
                    <button onClick={() => setRenamingId(null)} className="p-2 text-gray-400 hover:text-white" aria-label="Cancelar" title="Cancelar"><X size={16} /></button>
                  </>
                ) : (
                  <>
