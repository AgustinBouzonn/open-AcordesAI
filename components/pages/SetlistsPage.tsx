    ...
    <div className="space-y-6">
      ...
      {creating && (
        <div className="bg-dark-800 border border-dark-700 rounded-xl p-4 flex gap-2">
          ...
          />
          <button onClick={create} className="bg-brand hover:bg-brand/90 text-white px-4 rounded-lg" aria-label="Crear setlist" title="Crear setlist"><Check size={16} /></button>
          <button onClick={() => { setCreating(false); setNewName(''); }} className="bg-dark-700 hover:bg-dark-600 text-gray-300 px-4 rounded-lg" aria-label="Cancelar" title="Cancelar"><X size={16} /></button>
        </div>
      )}
            ...
            <div key={sl.id} className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
              <div className="p-4 flex items-center gap-3">
                {renamingId === sl.id ? (
                  ...
                  <button onClick={() => openId === sl.id ? setOpenId(null) : void loadDetail(sl.id)} className="flex-1 text-left" aria-expanded={openId === sl.id}>
                ...
                {renamingId === sl.id ? (
                  <>
                    <button onClick={() => void submitRename(sl.id)} className="p-2 text-emerald-400 hover:text-emerald-300" aria-label="Confirmar cambio de nombre" title="Confirmar"><Check size={16} /></button>
                    <button onClick={() => setRenamingId(null)} className="p-2 text-gray-400 hover:text-white" aria-label="Cancelar cambio de nombre" title="Cancelar"><X size={16} /></button>
                  </>
                ) : (
                  <>
                    <button onClick={() => { setRenamingId(sl.id); setRenameValue(sl.name); }} className="p-2 text-gray-400 hover:text-white" aria-label="Renombrar" title="Renombrar"><Pencil size={16} /></button>
                    <button onClick={() => void remove(sl.id)} className="p-2 text-gray-400 hover:text-red-400" aria-label="Eliminar" title="Eliminar"><Trash2 size={16} /></button>
                  </>
                )}
              </div>
                ...
                <div className="border-t border-dark-700 bg-dark-900/50 p-3 space-y-2">
                  ...
                  {detail.songs.length === 0 ? (
                    ...
                    <div key={song.id} className="bg-dark-800 rounded-lg p-2 flex items-center gap-2">
                      ...
                      <button onClick={() => void move(idx, -1)} disabled={idx === 0} className="p-1 text-gray-500 hover:text-white disabled:opacity-30" aria-label="Mover arriba" title="Mover arriba"><ChevronUp size={14} /></button>
                      <button onClick={() => void move(idx, 1)} disabled={idx === detail.songs.length - 1} className="p-1 text-gray-500 hover:text-white disabled:opacity-30" aria-label="Mover abajo" title="Mover abajo"><ChevronDown size={14} /></button>
                      <button onClick={() => void removeSong(sl.id, song.id)} className="p-1 text-gray-500 hover:text-red-400" aria-label="Quitar canción" title="Quitar canción"><Trash2 size={14} /></button>
                    </div>
                  ))}
                </div>
              )}
