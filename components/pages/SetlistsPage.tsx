    ...
    <div className="space-y-6">
      ...
      {creating && (
        <div className="bg-dark-800 border border-dark-700 rounded-xl p-4 flex gap-2">
          ...
          />
          <button onClick={create} aria-label="Guardar setlist" title="Guardar setlist" className="bg-brand hover:bg-brand/90 text-white px-4 rounded-lg"><Check size={16} /></button>
          <button onClick={() => { setCreating(false); setNewName(''); }} aria-label="Cancelar" title="Cancelar" className="bg-dark-700 hover:bg-dark-600 text-gray-300 px-4 rounded-lg"><X size={16} /></button>
        </div>
      )}
          ...
          {list.map((sl) => (
            <div key={sl.id} className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
              <div className="p-4 flex items-center gap-3">
                ...
                {renamingId === sl.id ? (
                  <>
                    <button onClick={() => void submitRename(sl.id)} aria-label="Guardar nombre" title="Guardar nombre" className="p-2 text-emerald-400 hover:text-emerald-300"><Check size={16} /></button>
                    <button onClick={() => setRenamingId(null)} aria-label="Cancelar" title="Cancelar" className="p-2 text-gray-400 hover:text-white"><X size={16} /></button>
                  </>
                ) : (
                  <>
                    <button onClick={() => { setRenamingId(sl.id); setRenameValue(sl.name); }} aria-label="Renombrar setlist" title="Renombrar setlist" className="p-2 text-gray-400 hover:text-white"><Pencil size={16} /></button>
                    <button onClick={() => void remove(sl.id)} aria-label="Eliminar setlist" title="Eliminar setlist" className="p-2 text-gray-400 hover:text-red-400"><Trash2 size={16} /></button>
                  </>
                )}
              </div>
                ...
                <div className="border-t border-dark-700 bg-dark-900/50 p-3 space-y-2">
                  ...
                  {detail.songs.length === 0 ? (
                    ...
                    <div key={song.id} className="bg-dark-800 rounded-lg p-2 flex items-center gap-2">
                      ...
                      <button onClick={() => void move(idx, -1)} disabled={idx === 0} aria-label="Mover arriba" title="Mover arriba" className="p-1 text-gray-500 hover:text-white disabled:opacity-30"><ChevronUp size={14} /></button>
                      <button onClick={() => void move(idx, 1)} disabled={idx === detail.songs.length - 1} aria-label="Mover abajo" title="Mover abajo" className="p-1 text-gray-500 hover:text-white disabled:opacity-30"><ChevronDown size={14} /></button>
                      <button onClick={() => void removeSong(sl.id, song.id)} aria-label="Eliminar canción" title="Eliminar canción" className="p-1 text-gray-500 hover:text-red-400"><Trash2 size={14} /></button>
                    </div>
                  ))}
                </div>
              )}
