    ...
    <div className="space-y-6">
      ...
      {creating && (
        <div className="bg-dark-800 border border-dark-700 rounded-xl p-4 flex gap-2">
          ...
          />
          <button onClick={create} className="bg-brand hover:bg-brand/90 text-white px-4 rounded-lg" aria-label="Crear" title="Crear"><Check size={16} /></button>
                ...
                <div className="border-t border-dark-700 bg-dark-900/50 p-3 space-y-2">
                  ...
                  {detail.songs.length === 0 ? (
                    ...
                    <div key={song.id} className="bg-dark-800 rounded-lg p-2 flex items-center gap-2">
                      ...
                      <button onClick={() => void move(idx, -1)} disabled={idx === 0} className="p-1 text-gray-500 hover:text-white disabled:opacity-30" aria-label="Subir canción" title="Subir canción"><ChevronUp size={14} /></button>
                      <button onClick={() => void move(idx, 1)} disabled={idx === detail.songs.length - 1} className="p-1 text-gray-500 hover:text-white disabled:opacity-30" aria-label="Bajar canción" title="Bajar canción"><ChevronDown size={14} /></button>
                      <button onClick={() => void removeSong(sl.id, song.id)} className="p-1 text-gray-500 hover:text-red-400" aria-label="Eliminar canción" title="Eliminar canción"><Trash2 size={14} /></button>
                    </div>
                  ))}
                </div>
              )}
