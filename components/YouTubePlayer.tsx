    ...
    <div className="bg-dark-800 border border-dark-700 rounded-xl mb-4 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 text-sm font-medium text-gray-300">
        ...
        {canEdit && videoId && !editing && (
          <div className="flex gap-1">
            <button onClick={() => { setEditing(true); setDraft(url); }} className="p-1 text-gray-400 hover:text-white" title="Editar" aria-label="Editar URL de YouTube"><Edit2 size={14} /></button>
            <button onClick={remove} disabled={saving} className="p-1 text-gray-400 hover:text-red-400" title="Quitar" aria-label="Quitar URL de YouTube"><Trash2 size={14} /></button>
          </div>
        )}
      </div>
      ...
      {canEdit && (editing || !videoId) && (
        <div className="flex gap-2 p-3 border-t border-dark-700">
          ...
          />
          <button onClick={save} disabled={saving || !draft.trim()} className="bg-brand hover:bg-brand/90 text-white px-3 rounded disabled:opacity-50" aria-label="Guardar URL de YouTube"><Check size={16} /></button>
          {videoId && <button onClick={() => { setEditing(false); setDraft(''); }} className="bg-dark-700 hover:bg-dark-600 text-gray-300 px-3 rounded" aria-label="Cancelar edición de URL de YouTube"><X size={16} /></button>}
        </div>
      )}
    </div>
  );
