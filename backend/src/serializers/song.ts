...
export const serializeSong = (row: SongRow) => {
  // ⚡ Bolt: Use manual object construction to avoid object allocation and array operations
  const res: Record<string, unknown> = {};
  if (row.id !== undefined) res.id = row.id;
  if (row.title !== undefined) res.title = row.title;
  if (row.artist !== undefined) res.artist = row.artist;
