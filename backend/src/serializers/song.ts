...
export const serializeSong = (row: SongRow) => {
  ...
  // Replaced `Object.fromEntries(Object.entries({...}).filter(...))` with manual object
  // construction and conditional property assignments to eliminate significant allocation
  // overhead of intermediate objects and arrays during serialization.
  // This drastically reduces garbage collection pauses in endpoints returning large lists of songs.
  ...
  const youtubeUrl = row.youtube_url ?? row.youtubeUrl ?? undefined;
  if (youtubeUrl !== undefined) result.youtubeUrl = youtubeUrl;
