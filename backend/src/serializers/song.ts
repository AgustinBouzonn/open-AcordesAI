...
export const serializeSong = (row: SongRow) => {
  ...
  if (lyrics !== undefined && lyrics !== null) result.lyrics = lyrics;

  const chords = row.chords;
  if (chords !== undefined && chords !== null) result.chords = chords;

  const author = row.author;
  if (author !== undefined && author !== null) result.author = author;
