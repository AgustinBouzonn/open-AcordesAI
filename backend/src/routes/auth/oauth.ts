...
async function handleOAuthCallback(provider: OAuthProvider, req: Request, res: Response): Promise<void> {
  ...
  // 🛡️ Sentinel: Validate OAuth state parameter against the OAUTH_COOKIE to prevent CSRF attacks
  const cookies = parseCookies(req.headers.cookie);
  const storedState = cookies[OAUTH_COOKIE];
  if (!storedState || state !== storedState) {
    redirectWithAuthResult(req, res, { error: 'Estado de OAuth inválido (posible CSRF)' });
    return;
  }
