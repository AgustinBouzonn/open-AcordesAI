...
export default function createSongsRouter(): Router {
  ...
  router.get('/popular', async (req: Request, res: Response) => {
    try {
      ...
      // ⚡ Bolt: Replaced multiple correlated subqueries with LEFT JOIN LATERAL to consolidate aggregation scans.
      const result = await query(
        `SELECT s.*, u.username as author,
         ...
         LEFT JOIN LATERAL (SELECT COUNT(*) as fav_count FROM favorites WHERE song_id = s.id) f ON true
         LEFT JOIN LATERAL (SELECT COUNT(*) as view_count FROM history WHERE song_id = s.id) h ON true
         LEFT JOIN LATERAL (SELECT COUNT(*) as has_chords FROM chord_cache WHERE song_id = s.id) c ON true
  ...
  router.get('/', async (req: Request, res: Response) => {
    try {
      ...
      // ⚡ Bolt: Replaced correlated subqueries with LEFT JOIN LATERAL for consolidated aggregation scans.
      const result = await query(
        `SELECT s.*, u.username as author, 
         r.rating,
         COALESCE(r.rating_count, 0) as rating_count,
