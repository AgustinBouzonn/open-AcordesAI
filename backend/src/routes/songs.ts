    ...
    try {
      ...
      const result = await query(
        `SELECT s.*, u.username as author,
         COALESCE(r_stats.rating, 0) as rating,
         ...
         -- Optimize aggregations using LATERAL joins
         LEFT JOIN LATERAL (
           SELECT AVG(score) as rating, COUNT(*) as rating_count FROM ratings WHERE song_id = s.id
         ) r_stats ON true
         LEFT JOIN LATERAL (
           SELECT COUNT(*) as fav_count FROM favorites WHERE song_id = s.id
         ) f_stats ON true
         LEFT JOIN LATERAL (
           SELECT COUNT(*) as view_count FROM history WHERE song_id = s.id
         ) h_stats ON true
         LEFT JOIN LATERAL (
           SELECT COUNT(*) as has_chords FROM chord_cache WHERE song_id = s.id
         ) c_stats ON true
         WHERE cc.id IS NOT NULL
         ORDER BY r_stats.rating_count DESC, f_stats.fav_count DESC, h_stats.view_count DESC
         LIMIT $1`,
        [limit]
      );
