...
export default function createSongsRouter(): Router {
  ...
  router.get('/', async (req: Request, res: Response) => {
    try {
      ...
      // ⚡ Bolt Optimization: Use LEFT JOIN LATERAL to eliminate N+1 correlated subqueries while
      // preserving condition pushdown. Consolidates multiple ratings aggregates into a single query.
      const result = await query(
        `SELECT s.*, u.username as author, 
         r.avg_score as rating,
         COALESCE(r.cnt, 0) as rating_count,
         COALESCE(cc.cnt, 0) as has_chords
         FROM songs s 
         LEFT JOIN users u ON s.user_id = u.id
