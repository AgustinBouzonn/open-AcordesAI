...
export default function createSongsRouter(): Router {
  ...
  router.get('/', async (req: Request, res: Response) => {
    try {
      ...
      // ⚡ Bolt Performance Optimization: Used LEFT JOIN LATERAL instead of multiple SELECT subqueries
      // to combine ratings aggregations into a single scan per row and preserve early limits.
      const result = await query(
        `SELECT s.*, u.username as author, 
         r.rating,
         r.rating_count,
