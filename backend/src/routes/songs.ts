...
export default function createSongsRouter(): Router {
  ...
  router.get('/popular', async (req: Request, res: Response) => {
    try {
      ...
      // Optimize: Replace correlated subqueries with LEFT JOIN LATERAL to compute aggregations in a single scan per row
      const result = await query(
        `SELECT s.*, u.username as author,
         ...
         INNER JOIN chord_cache cc ON s.id = cc.song_id
