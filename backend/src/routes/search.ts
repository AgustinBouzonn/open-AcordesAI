  ...
  try {
    ...
    const result = await query(
      `SELECT s.*,
       ...
       -- Replaced correlated subqueries in SELECT clause with LEFT JOIN LATERAL.
       -- This resolves N+1 query execution problems and allows computing both
       -- rating avg and count in a single pass.
       LEFT JOIN LATERAL (
