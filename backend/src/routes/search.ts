...
router.get('/local', async (req: Request, res: Response) => {
  ...
  try {
    const like = `%${q}%`;
    // ⚡ Bolt Performance Optimization: Replaced correlated subqueries in SELECT with LEFT JOIN LATERAL to preserve index usage for specific queried rows and avoid N+1 overhead
    const result = await query(
      `SELECT s.*,
              u.username as author,
              r.rating, r.rating_count, c.has_chords
       FROM songs s
