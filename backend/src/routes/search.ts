...
router.get('/local', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await query(
      /* ⚡ Bolt Performance Optimization: Replace N+1 correlated subqueries in SELECT and ORDER BY with LEFT JOIN LATERAL for better execution plans on filtered rows */
      `SELECT s.*,
