...
// --- Extracted Views for Performance ---
// ⚡ Bolt: By defining these components outside of AppContent, we prevent them from being
// recreated on every render of AppContent (e.g., when the search bar input changes).
// This avoids unnecessary processing (like calling getFavoriteSongsFull() parsing local storage)
// for routes that are not currently active, significantly reducing re-renders and CPU usage.

const HomeView = ({ searchQuery, setSearchQuery, handleSearch }: {
  searchQuery: string,
  setSearchQuery: (query: string) => void,
  handleSearch: (e: React.FormEvent | { preventDefault: () => void }) => Promise<void> | void
}) => {
  return (
...
const SearchView = ({
  searchQuery, setSearchQuery, handleSearch, isSearching, searchResults, loadSong
}: {
  searchQuery: string,
  setSearchQuery: (query: string) => void,
...
const FavoritesView = ({ loadSong, navigate }: { loadSong: (id: string) => Promise<void> | void, navigate: NavigateFunction }) => {
...
const HistoryView = ({ loadSong }: { loadSong: (id: string) => Promise<void> | void }) => {
...
const SongDetailView = ({
  isLoadingSong, currentSong, handleInstrumentChange, loadingInstrument
}: {
