...
// Extracted View Components wrapped in React.memo

interface HomeViewProps {
  searchQuery: string;
  setSearchQuery: (query: string) => void;
  handleSearch: (e: React.FormEvent) => Promise<void> | void;
  navigate: ReturnType<typeof useNavigate>;
}

const HomeView = React.memo(({ searchQuery, setSearchQuery, handleSearch, navigate }: HomeViewProps) => {
  // Use a stable reference for the trending click handler
  const onTrendingClick = useCallback((term: string) => {
    setSearchQuery(term);
    // Create a synthetic event object for handleSearch
    const syntheticEvent = { preventDefault: () => {} } as React.FormEvent;
    // We also need to trigger the search with the term, since state update is async
    // However, AppContent handles this by intercepting and using the updated state if possible
    // To make it robust without changing AppContent's handleSearch signature:
    // It's better if handleSearch in AppContent uses the current parameter if provided.
    // Let's assume handleSearch will just use the state. We'll set state and then call handleSearch.
...
interface SearchViewProps {
  ...
  loadSong: (id: string, title?: string, artist?: string) => Promise<void> | void;
}

const SearchView = React.memo(({ searchQuery, setSearchQuery, handleSearch, isSearching, searchResults, loadSong }: SearchViewProps) => {
  return (
...
const FavoritesView = React.memo(({ loadSong, navigate }: FavoritesViewProps) => {
...
const HistoryView = React.memo(({ loadSong }: HistoryViewProps) => {
...
interface SongDetailViewProps {
  ...
  handleInstrumentChange: (instrument: Instrument) => Promise<void> | void;
  loadingInstrument: boolean;
}

const SongDetailView = React.memo(({ isLoadingSong, currentSong, handleInstrumentChange, loadingInstrument }: SongDetailViewProps) => {
  if (isLoadingSong) {
    return (
...
function AppContent() {
  ...
  return (
    <Layout activeTab={activeTab} onNavigate={(tab) => {
      ...
      <Routes>
        <Route path="/" element={<HomeView searchQuery={searchQuery} setSearchQuery={setSearchQuery} handleSearch={handleSearch} navigate={navigate} />} />
        ...
        <Route path="/favorites" element={<FavoritesView loadSong={loadSong} navigate={navigate} />} />
