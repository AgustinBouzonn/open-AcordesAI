...
// ⚡ Bolt: Extracted route views to standalone memoized components to prevent spurious evaluations on unrelated state changes (like search input keystrokes).
...
const FavoritesView = React.memo(({
  ...
  loadSong: (id: string) => Promise<void> | void,
...
const SongDetailView = React.memo(({
  isLoadingSong,
  currentSong,
...
function AppContent() {
  ...
  // Handle Search
  const handleSearch = useCallback(async (e: React.FormEvent | { preventDefault: () => void }) => {
    e.preventDefault();
