...
// --- Extracted Memoized Item Components ---

const TrendingItem = memo(({ term, onClick }: { term: string; onClick: (term: string) => void }) => (
  <div
...
const SearchResultItem = memo(({ result, onClick }: { result: SongSearchResult; onClick: (id: string, title?: string, artist?: string) => Promise<void> | void }) => (
  <div
    onClick={() => onClick(result.id, result.title, result.artist)}
...
const FavoriteItem = memo(({ song, onClick }: { song: Song; onClick: (id: string) => Promise<void> | void }) => (
  <div
...
const HistoryItem = memo(({ song, onClick }: { song: Song; onClick: (id: string) => Promise<void> | void }) => (
  <div
    onClick={() => onClick(song.id)}
...
// --- Extracted Memoized View Components ---
...
const HomeView = memo(({
  ...
  <div className="space-y-12 animate-in fade-in duration-500">
    <div className="text-center space-y-6 pt-8 pb-4">
      <div className="inline-flex items-center gap-2 bg-dark-800 px-4 py-2 rounded-full border border-dark-700 text-sm font-medium text-gray-300 mb-4 shadow-sm">
      ...
      <h1 className="text-4xl md:text-6xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-brand to-yellow-500">
        Toca lo que quieras.
      </h1>
      ...
      <form onSubmit={handleSearchFormSubmit} className="max-w-md mx-auto relative">
        <input
          type="text"
          value={searchQuery}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {TRENDING_SEARCHES.map((term, idx) => (
          <TrendingItem key={idx} term={term} onClick={onTrendingClick} />
        ))}
      </div>
    </div>
  </div>
     ...
     {searchResults.length > 0 ? (
       <div className="space-y-2">
         {searchResults.map((result) => (
           <SearchResultItem key={result.id} result={result} onClick={loadSong} />
         ))}
       </div>
     ) : (
       !isSearching && (
...
const FavoritesView = memo(({ navigate, loadSong }: { navigate: any; loadSong: (id: string) => Promise<void> | void }) => {
    ...
    <div className="space-y-6">
      ...
      {favs.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {favs.map((song) => (
            <FavoriteItem key={song.id} song={song} onClick={loadSong} />
          ))}
        </div>
      )}
    </div>
...
const HistoryView = memo(({ loadSong }: { loadSong: (id: string) => Promise<void> | void }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {history.length === 0 ? (
        ...
        <div className="space-y-2">
          {history.map((song) => (
            <HistoryItem key={song.id} song={song} onClick={loadSong} />
          ))}
        </div>
      )}
    </div>
...
function AppContent() {
  ...
  const handleSearchFormSubmit = useCallback((e: React.FormEvent) => {
    e.preventDefault();
