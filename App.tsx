...
// ⚡ Bolt Performance Optimization: Extract list items into React.memo components
// This prevents unnecessary re-renders of the entire list when unrelated state in AppContent changes.
...
const SearchResultItem = memo(({ result, onClick }: { result: SongSearchResult; onClick: (id: string, title?: string, artist?: string) => void }) => (
  <div
...
const FavoriteItem = memo(({ song, onClick }: { song: Song; onClick: (id: string) => void }) => (
  <div
...
const HistoryItem = memo(({ song, onClick }: { song: Song; onClick: (id: string) => void }) => (
  <div
    onClick={() => onClick(song.id)}
...
function AppContent() {
  ...
  // ⚡ Bolt Performance Optimization: Abstract search logic and wrap handlers in useCallback
  // This provides stable references for child components, preventing unnecessary re-renders.
