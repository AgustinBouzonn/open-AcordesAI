...
// Extracted inline views to separate memoized components outside of AppContent.
// Previously, these views (like renderFavorites) were defined as inline functions within AppContent.
// This caused React Router to re-evaluate them on every state change (e.g., when typing in the search bar, updating `searchQuery`),
// leading to expensive synchronous `localStorage` reads on the main thread, even when the view was inactive.
// Wrapping them in React.memo() ensures they only re-render when their specific props change.

const HomeView = React.memo(({ searchQuery, setSearchQuery, handleSearch }: { searchQuery: string, setSearchQuery: (q: string) => void, handleSearch: (e: React.FormEvent) => void }) => (
...
const SearchView = React.memo(({ searchQuery, setSearchQuery, handleSearch, isSearching, searchResults, loadSong }: { searchQuery: string, setSearchQuery: (q: string) => void, handleSearch: (e: React.FormEvent) => void, isSearching: boolean, searchResults: SongSearchResult[], loadSong: (id: string, title?: string, artist?: string) => void }) => (
...
const FavoritesView = React.memo(({ navigate, loadSong }: { navigate: (path: string) => void, loadSong: (id: string) => void }) => {
...
const HistoryView = React.memo(({ loadSong }: { loadSong: (id: string) => void }) => {
...
const SongDetailView = React.memo(({ isLoadingSong, currentSong, handleInstrumentChange, loadingInstrument }: { isLoadingSong: boolean, currentSong: Song | null, handleInstrumentChange: (instrument: Instrument) => Promise<void>, loadingInstrument: boolean }) => {
  if (isLoadingSong) {
    return (
...
function AppContent() {
  ...
  const handleInstrumentChange = useCallback(async (instrument: Instrument) => {
  ...
  return (
    <Layout activeTab={activeTab} onNavigate={(tab) => {
      ...
      <Routes>
        {/* --- ⚡ Bolt Performance Optimization --- */}
        {/* Using memoized view components instead of executing inline render functions here. */}
        {/* This avoids heavy operations like reading localStorage executing when active view isn't matching. */}
        <Route path="/" element={<HomeView searchQuery={searchQuery} setSearchQuery={setSearchQuery} handleSearch={handleSearch} />} />
        <Route path="/search" element={<SearchView searchQuery={searchQuery} setSearchQuery={setSearchQuery} handleSearch={handleSearch} isSearching={isSearching} searchResults={searchResults} loadSong={loadSong} />} />
